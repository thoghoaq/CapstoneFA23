...
class TrackingOrderPage extends StatelessWidget {
  final String id;
  const TrackingOrderPage({super.key, required this.id});

  String get _orderName => "Đơn hàng $id";
        ...
        body: FutureBuilder(
          ...
          builder: (context, snapshot) {
            ...
            ShippingProgress status = ShippingProgress.getMapTransactionStatus(
                provider.order.currentOrderStatus);
            bool inProgress = [
            ...
            dynamic order = {
              ...
              "title": "Được gửi bởi ${provider.order.senderName}",
              "subtitle": "${provider.order.senderPhoneNumber}",
              "description": inProgress
                  ? "Ngày nhận dự kiến: ${DateTimeHelper.getDateTime(provider.order.expectedShippingDate)}"
                  : "Đã nhận hàng",
              "status": provider.order.currentOrderStatus,
              "note": provider.order.note
            };
            return Consumer<OrderProvider>(
              builder: (context, value, child) {
                return Column(children: [
                  ...
                  if (status == ShippingProgress.feedback)
                    Expanded(
                      child: Column(
                  ...
                  Padding(
                    ...
                    child: Row(
                      children: [
                        if (order["icon"] != null)
                          DAvatarCircle(
                              image: Image.asset(order["icon"]), radius: 41),
